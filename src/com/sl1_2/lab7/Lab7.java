...
/**
 * A student object to store the information of each student
 */
class Student {
    int studentId;
    int grade;
...
public class Lab7 {
    ...
    private static void printStudentInfo(Student[] studentList, int studentCount) {
        ...
        for (int i = 0; i < studentCount; i++) {
            System.out.printf("%d %-2d %16s %16s %s\n", studentList[i].studentId, studentList[i].grade,
                              studentList[i].lastName, studentList[i].firstName, studentList[i].gender);
        }
    }
    ...
    private static void studentInfoTally(Student[] studentList, int studentCount) {
        ...
        while (i < studentCount) {
            ...
            if (studentList[i].grade == 9) {
                grades[0]++;
            } else if (studentList[i].grade == 10) {
                grades[1]++;
            } else if (studentList[i].grade == 11) {
                grades[2]++;
            } else if (studentList[i].grade == 12) {
                grades[3]++;
            }

            if (studentList[i].gender.equals("Male")) {
                gender[0]++;
            } else if (studentList[i].gender.equals("Female")) {
                gender[1]++;
            }

            lastNameChar = (int) studentList[i].lastName.charAt(0);
    ...
    private static void sortStudents(Student[] studentList, int studentCount, boolean sortByName) {
        ...
        for (j = 0; j < studentCount - 1; j++) {
            for (i = 0; i < studentCount - 1; i++) {
                ...
                if (sortByName) {
                    compare1  = (int) studentList[i].lastName.charAt(0);
                    compare2 = (int) studentList[i + 1].lastName.charAt(0);
                    fileName = "className.txt";
                } else {
                    compare1 = studentList[i].studentId;
                    compare2 = studentList[i + 1].studentId;
        ...
        try (FileWriter fw = new FileWriter(fileName)) {
            for (i = 0; i < studentCount; i++) {
                fw.write(studentList[i].studentId + " " + studentList[i].grade + " " + studentList[i].lastName +
                        " " + studentList[i].firstName + " " + studentList[i].gender + "\n");
            }
        } catch (IOException e) {
